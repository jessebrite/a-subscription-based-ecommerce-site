...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if (filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)) {
		...
		$sql = 'SELECT id FROM users WHERE email = "' .  mysqli_real_escape_string ($dbc, $_POST['email']) . '"';
	...
	if (empty($pass_errors)) { // If everything's OK.
		...
		$pass = substr(md5(uniqid(rand(), true)), 15, 15);

		// Update the database:
		$sql = "UPDATE users SET pass = '" .  get_password_hash($pass) . "' WHERE id = $uid LIMIT 1";
		$result = mysqli_query ($dbc, $sql);
		
		if (mysqli_affected_rows($dbc) == 1) { // If it ran OK.
		
			// Send an email:
			$body = "Your password to log into <whatever site> has been temporarily changed to '$pass'. Please log in using that password and this email address. Then you may change your password to something more familiar.";
