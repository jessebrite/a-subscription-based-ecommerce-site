...
$sql = 'SELECT pages.id, title, description, DATE_FORMAT(history.date_created, "%M %d, %Y") FROM pages LEFT JOIN history ON (pages.id = history.page_id) WHERE history.user_id = ' . $_SESSION['user_id'] . ' GROUP BY (history.page_id) ORDER BY history.date_created DESC';
...
// Get the pages they've viewed
$sql = 'SELECT pdfs.tmp_name, title, description, DATE_FORMAT(history.date_created, "%M %d, %Y") FROM pdfs LEFT JOIN history ON (pdfs.id = history.pdf_id) WHERE history.user_id = ' . $_SESSION['user_id'] . ' GROUP BY (history.pdf_id) ORDER BY history.date_created DESC';
$result = mysqli_query($dbc, $sql);
