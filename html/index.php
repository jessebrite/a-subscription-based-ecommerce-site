...
require ('./includes/config.inc.php'); # the HTML includes file was used
...
$sql = "SELECT COUNT(history.id) AS num, pages.id, pages.title FROM pages, history WHERE pages.id = history.page_id AND history.type = 'page' GROUP BY (history.page_id) ORDER BY num DESC LIMIT 3";
...
$sql = "SELECT ROUND(AVG(rating),1) AS average, pages.id, pages.title FROM pages, page_ratings WHERE pages.id=page_ratings.page_id GROUP BY (page_ratings.page_id) ORDER BY average DESC LIMIT 3";
$result = $dbc->query($sql);
