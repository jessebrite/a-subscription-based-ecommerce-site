...
require_once ('./includes/config.inc.php'); # the HTML includes file was used
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if (preg_match ('/^[A-Z \'.-]{2,20}$/i', ($_POST['first_name']))) {
		$firstname = ucfirst(strtolower(mysqli_real_escape_string($dbc, $_POST['first_name'])));
	} else {
	...
	if (preg_match ('/^[A-Z \'.-]{2,40}$/i', $_POST['last_name'])) {
		$lastname = ucfirst(strtolower(mysqli_real_escape_string($dbc, $_POST['last_name'])));
	} else {
	...
	if (empty($reg_errors)) { // If everything's OK...
		...
		if ($rows == 0) { // No problems!
			...
			$sql = "INSERT INTO users (username, email, pass, first_name, last_name, 
					date_expires) VALUES (?,?, '" . get_password_hash($pass) . "', ?,?, 
					ADDDATE(NOW(), INTERVAL 1 MONTH) )";
			if ($stmt->prepare($sql)) {
				$stmt->bind_param('ssss', $username, $email, $firstname, $lastname);
				$stmt->execute();
