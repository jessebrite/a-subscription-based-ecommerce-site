...
if (isset($_GET['id']) && (strlen($_GET['id']) == 40) && (substr($_GET['id'], 0, 1) != '.') ) {
	...
	if (file_exists ($file) && is_file($file)) {
		...
		// Get the info:
		$sql = 'SELECT id, title, description, file_name FROM pdfs WHERE tmp_name = "' . mysqli_real_escape_string($dbc, $_GET['id']) . '"';
		...
		if (mysqli_num_rows($result) == 1) { // Ok!
			...
			if (isset($_SESSION['user_not_expired'])) {
				...
				$sql = "INSERT INTO history (user_id, type, pdf_id) VALUES ({$_SESSION['user_id']}, 'pdf', {$row['id']})";
				$result = mysqli_query($dbc, $sql);
